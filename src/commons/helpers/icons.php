...
/**
 * Retourne les familles de police d'icônes
 * - Traitement des fichiers JSON
 * - Mise en cache des traitements lorsque c'est demandé
...
/**
 * Parse le fichier json du générateur IcoMoon
 * Pour en savoir plus : https://icomoon.io/app/#/select
 */
function woodkit_parse_icomoon_icons_json ($json) {
	$res = array();
	if ($json && isset($json['icons'])) {
		$icons = array();
		$icons_set_prefix = isset($json['preferences']) && isset($json['preferences']['fontPref']) && isset($json['preferences']['fontPref']['prefix']) ? $json['preferences']['fontPref']['prefix'] : 'icomoon-';
		$icons_set_name = isset($json['preferences']) && isset($json['preferences']['fontPref']) && isset($json['preferences']['fontPref']['metadata']) && isset($json['preferences']['fontPref']['metadata']['fontFamily']) ? $json['preferences']['fontPref']['metadata']['fontFamily'] : 'Icomoon';
		foreach ($json['icons'] as $icon) {
			$icon_label = isset($icon['properties']) && isset($icon['properties']['name']) ? $icon['properties']['name'] : 'Unknown';
			$icon_value = isset($icon['properties']) && isset($icon['properties']['name']) ? $icon['properties']['name'] : null;
			$icon_search = str_replace('-', ' ', $icon_label);
			if ($icon_value) {
				$icons[] = array(
						'value' => $icons_set_prefix.$icon_value,
						'label' => $icon_label,
						'search' => $icon_search
				);
			}
