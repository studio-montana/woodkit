...
function tool_media_is_active($active){
	$active_option = woodkit_get_option('tool-media-active');
	if (!empty($active_option) && $active_option == "on")
		$active = true;
	return $active;
}
add_filter("woodkit_is_tool_media_active", "tool_media_is_active", 1, 1);
...
function tool_media_get_config_options_section_description(){
	echo '<p class="tool-description">'.tool_media_get_description().'</p>';
}
...
function tool_media_get_config_options_field_active($args){
	$options = $args['options'];
	$active = false;
	$value = "off";
	if (isset($options['tool-media-active']))
		$value = $options['tool-media-active'];
	$checked = '';
	if ($value == 'on')
