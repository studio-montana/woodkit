(function() {
	tinymce.create('tinymce.plugins.toolshortcodesexergue', {
		/**
		...
		init : function(ed, url) {
			ed.addButton('toolshortcodesexergue', {
				title : 'Exergue',
				cmd : 'toolshortcodesexergue',
				icon : 'none fa fa-star'
			});

			ed.addCommand('toolshortcodesexergue', function() {
				var return_text = '[exergue]';
				if (!empty(ed.selection.getContent())) {
		...
		getInfo : function() {
			return {
				longname : 'Woodkit Exergue Shortcode',
	...
	// Register plugin
	tinymce.PluginManager.add('toolshortcodesexergue', tinymce.plugins.toolshortcodesexergue);
})();