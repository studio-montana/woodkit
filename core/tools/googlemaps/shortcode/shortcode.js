	...
	tinymce.create('tinymce.plugins.toolgooglemapsshortcode', {
		...
		init : function(ed, url) {
			...
			ed.addCommand('toolgooglemapsshortcode', function() {
				tool_googlemaps_shortcode_open(ed, ed.selection.getContent(), function(ed, ed_selection, id, adress, title, zoom, type, width, height, style) {
					var return_text = '[googlemaps id="'+id+'" adress="'+adress+'" title="'+title+'" zoom="'+zoom+'" type="'+type+'" width="'+width+'" height="'+height+'" style="'+style+'" /]';
