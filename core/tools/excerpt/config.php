...
function tool_excerpt_is_active($active){
	$active_option = woodkit_get_option('tool-excerpt-active');
	if (!empty($active_option) && $active_option == "on")
		$active = true;
	return $active;
}
add_filter("woodkit_is_tool_excerpt_active", "tool_excerpt_is_active", 1, 1);
...
function tool_excerpt_get_config_options_section_documentation_url(){
	return WOODKIT_URL_DOCUMENTATION.'/resume-personnalise';
}
