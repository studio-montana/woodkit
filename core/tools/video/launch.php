...
function video_get_featured_video($post_id = null, $width = "", $height="", $args = array()){
	if ($post_id == null)
		$post_id = get_the_ID();
	if (empty($width)){
		$width = woodkit_get_tool_option(VIDEO_TOOL_NAME, "default-width");
	}
	if (empty($height)){
		$height = woodkit_get_tool_option(VIDEO_TOOL_NAME, "default-height");
	}
	$res = '';
...
function tool_video_post_thumbnail_filter($html, $post_id, $post_thumbnail_id, $size, $attr){
	$auto_insert = woodkit_get_tool_option(VIDEO_TOOL_NAME, "auto-insert");
