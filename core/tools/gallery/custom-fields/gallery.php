...
define('META_GALLERY_PRESENTATION_MASONRY_WIDTH_WOODKITIZED', 'meta_gallery_initial_max_width_customized');
...
function gallery_add_inner_meta_boxes($post){
	...
	if ($id_blog_page != get_the_ID()){
		...
		if (in_array(get_post_type($post), $available_posttypes)){
			include(locate_ressource('/'.WOODKIT_PLUGIN_TOOLS_FOLDER.GALLERY_TOOL_NAME.'/custom-fields/templates/gallery.php'));
		}
	}
}
...
function gallery_save_post($post_id){
	...
	if (in_array(get_post_type($post_id), $available_posttypes)){
		...
		// META_GALLERY_PRESENTATION_MASONRY_WIDTH_WOODKITIZED
		if (!empty($_POST[META_GALLERY_PRESENTATION_MASONRY_WIDTH_WOODKITIZED])){
			update_post_meta($post_id, META_GALLERY_PRESENTATION_MASONRY_WIDTH_WOODKITIZED, sanitize_text_field($_POST[META_GALLERY_PRESENTATION_MASONRY_WIDTH_WOODKITIZED]));
		}else{
			update_post_meta($post_id, META_GALLERY_PRESENTATION_MASONRY_WIDTH_WOODKITIZED, "250");
		}
		// META_GALLERY_PRESENTATION_MASONRY_HEIGHT
		if (!empty($_POST[META_GALLERY_PRESENTATION_MASONRY_HEIGHT]) && is_numeric($_POST[META_GALLERY_PRESENTATION_MASONRY_HEIGHT])){
			update_post_meta($post_id, META_GALLERY_PRESENTATION_MASONRY_HEIGHT, sanitize_text_field($_POST[META_GALLERY_PRESENTATION_MASONRY_HEIGHT]));
...
function gallery_post_gallery($output, $attr) {
	...
	if (in_array(get_post_type(get_the_ID()), get_displayed_post_types())){
		...
		$meta_gallery_presentation_masonry_width_customized = get_post_meta(get_the_ID(), META_GALLERY_PRESENTATION_MASONRY_WIDTH_WOODKITIZED, true);
		...
		if (!empty($attachments)){
			$gallery_post_count ++;
			ob_start();
			$gallery_template = locate_ressource(WOODKIT_PLUGIN_TOOLS_FOLDER.GALLERY_TOOL_NAME.'/templates/tool-gallery-display.php');
